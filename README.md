...
В структуре проекта для создания S3 BUCKET создан каталог s3-bucket/. 

После применения terraform манифестов в данном каталоге, в каталог files/ будут сохранены ключи доступа к bucket для чтения файла состояния.

Данные ключи можно экспортировать в специальные переменные окружения AWS_SECRET_KEY, AWS_ACCESS_KEY.

Возможность работы с вынесенным файлом состояния включается в backend.tf.
